...
<script type="text/javascript">
	function tiaoZhuan1()
	{
		if(document.getElementById("xiugai").value == "3")
		{
		    form3.submit();
		}
		else
		{
			alert("您的当前需求已经提交，不能修改");
		}
	}
 </SCRIPT>
<style type="text/css">
...
<body>
                   <%
	        		...
	        		{
	        		%>
<form id="form3" name="form3" method="post" action="tableXiugai.jsp">        		
<table width="981" height="507" border="1">
  ...
  if(rs.getString(17).equals("0"))
  {
	  %>
	  <tr id="形式审核意见">
	    <td colspan="2">形式审核结果</td>
	    <td colspan="3">提交未审核</td>
	  </tr>
	  <%
  }
  ...
  <tr id="形式审核意见">
    ...
    <td colspan="3"><%=rs.getString(20)%></td>
  </tr>
  <%
  if(rs.getString(18).equals("0"))
  {
	  ...
	  %>
	  <tr id="部门审核结果">
	    <td colspan="2">部门审核结果</td>
	    <td colspan="3">未通过形式审核</td>
	  </tr>
	  <%
	  }
	  else
  ...
  else if(rs.getString(18).equals("1"))
  {
	  %>
	  <tr id="部门审核结果">
	    <td colspan="2">部门审核结果</td>
	    <td colspan="3">部门审核通过</td>
	  </tr>
	  <%
  }
  else if(rs.getString(18).equals("3"))
  {
	  %>
	  <tr id="部门审核结果">
	    <td colspan="2">部门审核结果</td>
  ...
  <tr id="部门审核意见">
    ...
    <td colspan="3"><%=rs.getString(21)%></td>
  </tr>
  <tr>
        <td height="34" colspan="5"> 
          <INPUT TYPE="hidden"  id = "xiugai" name = "xiugai" value='<%=rs.getString(17)%>'>
          <INPUT TYPE="hidden"  id = "tijiaoxingshi" name = "tijiaoxingshi" value='<%=demandid%>'>
          <input type="button" name="button" id="button" value="修改" onclick="tiaoZhuan1()"/></td>
  </tr>
</table>
</form>
<% }%>
